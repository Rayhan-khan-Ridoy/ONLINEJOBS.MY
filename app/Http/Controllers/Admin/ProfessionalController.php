...
class ProfessionalController extends Controller
{
    ...
    public function getProfessionalsData()
    {
        if (auth()->user()->hasRole('superadministrator|administrator')) {
            try {
                $query = User::with('professional_profile')
                    ->where('status', 0)
                    ->whereRoleIs('professional');
                ...
                return DataTables::of($query)
                    ->addColumn('action', function ($user) {
                        ...
                        if (auth()->user()->hasRole('superadministrator')) {
                            $string .= '<a class="ml-1 btn btn-danger" href="' . route('admin.professional.block', $user->id) . '" onclick="return confirm(\'Are you sure?\')">Block</a>';
                        }
                        return $string;
                    })
                    ->addColumn('country', function ($user) {
                        return $user->professional_profile->job_seeker_country_data['name'] ?? 'N/A';
                    })
                    ->addColumn('city', function ($user) {
                        return $user->professional_profile->job_seeker_city_data['name'] ?? 'N/A';
                    })
                    ->addColumn('profile_image', function ($user) {
                        $img = $user->professional_profile['profile_image']
                            ? asset('storage/resume/' . $user->professional_profile['profile_image'])
                            : asset('images/dummy.jpg');
                        return '<img src="' . $img . '" border="0" width="40" class="img-rounded" align="center" />';
                    })
                    ->addColumn('name', function ($user) {
                        return $user->professional_profile['name'] ?? 'N/A';
                    })
                    ->addColumn('email', function ($user) {
                        return $user->professional_profile['email'] ?? 'N/A';
                    })
                    ->addColumn('created_at', function ($user) {
                        $date = $user->professional_profile['created_at'] ?? null;
                        return $date ? date("m/d/Y", strtotime($date)) : 'N/A';
                    })
                    ->addColumn('position_name', function ($user) {
                        return $user->professional_profile['job_category']
                            ? $user->professional_profile->job_seeker_job_category_data['name'] ?? 'N/A'
                            : 'N/A';
                    })
